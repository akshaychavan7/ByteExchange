...
const upvote = async (req, res) => {
    try {
        ...
        if (obj.upvoted_by.includes(req.userId)) {
            ...
            await voteObject.findByIdAndUpdate(id, { $pull: { downvoted_by: req.userId }, $inc: { vote_count: 1 }});
        } else {
...
const downvote = async (req, res) => {
    try {
        ...
        if (obj.downvoted_by.includes(req.userId)) {
            ...
            await voteObject.findByIdAndUpdate(id, { $pull: { upvoted_by: req.userId }, $inc: { vote_count: -1 }});
        } else {
            await voteObject.findByIdAndUpdate(id, { 
