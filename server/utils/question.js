...
const getQuestionsByOrder = async (order) => {
    let questions = await Question.find().populate('tags').populate('answers').populate({
      path: "asked_by",
    });
    ...
    if (order == "active") {
        questions.sort((a, b) => {
            const latestAnsDateTimeA = a.answers.length > 0 ?
            a.answers.reduce((latest, answer) => {
                return answer.ans_date_time > latest ? answer.ans_date_time : latest;
            }, new Date(0)) :
            new Date(0);
    
        const latestAnsDateTimeB = b.answers.length > 0 ?
...
const checkKeywordInQuestion = (question, searchKeyword) => {
    return searchKeyword.every((word) =>
